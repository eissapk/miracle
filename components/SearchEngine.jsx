    ...
    <div className="p-[2px]">
      <div className="w-full">
        <div className="o-search w-full before:right-[10px] before:left-auto">
          <input
            ...
            required
          />
          <button className="o-close !h-full !right-auto !left-0 !top-0 !bg-[position:3px_11px] [transform:rotate(180deg)]" onClick={() => setInput('')} />
        </div>
      </div>
      ...
      <div className="search-inner w-full min-h-[70px] mt-[5px] rounded-[3px] border border-[#e8dcc8]">
        <nav className="px-[10px] pl-[35px] pb-[5px] pt-[10px] border-b border-[#e8dcc8] relative bg-[#fdf8f0]">
          ...
          />
          {length > 0 && (
            <div className="text-xs text-[#666] font-tajawal tracking-[1px] mt-[5px] font-bold">
              {arNum(length)} نتيجة
            </div>
          )}
        </nav>
        ...
        {!settingsShown && hasResults && (
          <div>
            <ul className="m-0 p-[10px] list-none overflow-auto h-[calc(100vh-255px)] scrollbar" onClick={goTo} onScroll={handleScroll} ref={ulRef}>
              {tab === 'verse' && results.map((item, index) => (
                <li
                  key={index}
              ...
              {tab === 'surah' && results.map((item, index) => (
                <li
                  key={index}
                  className="search-result-item border-b border-[#eee] py-[10px] rounded-[3px] cursor-pointer overflow-hidden first:pt-0 last:border-0 last:pb-0"
                  data-surah={`${item.page}-${item.ayahs[0].globalVerse}`}
                >
                  <div className="pointer-events-none float-right">
                    <span className="text-xs font-bold text-[#666] font-kitab text-[20px]">سُورَةُ {item.name}</span>
                  </div>
                  <div className="w-[140px] float-left p-[3px]">
                    <label className="mb-[10px] font-tajawal font-bold text-[14px] text-[#666] pointer-events-none block">اختر الأية</label>
                    <div className="o-select max-w-[70px] inline-block mr-[10px] min-w-[70px]">
                      <select>
                        {item.ayahs.map((surah, i) => (
                          <option key={i} value={`${surah.page}-${surah.globalVerse}`} selected={i === 0}>
                            {arNum(surah.localVerse)}
                          </option>
                        ))}
                      </select>
                    </div>
        ...
        {!settingsShown && !hasResults && (
          <div className="search-no-results text-center p-[10px] text-[#666] text-[14px] tracking-normal select-none">
            {resultsHint}
          </div>
        )}
            ...
            <div className="p-0 px-[10px]">
              {tabs.map((item, index) => (!item.tab || (item.tab !== 'surah' && item.tab !== 'verse')) && (
                ...
                >
                  ...
                  <input
                    type="checkbox"
                    className="o-switch-btn"
                    checked={item.checked}
                    onChange={() => handleCheckedTabs(item)}
                  />
                </label>
              ))}
            </div>
